...
NTRU+ can be used in a variety of protocol settings. As a KEM, it can replace ephemeral Diffie-Hellman in authenticated key exchange protocols, including deployments in TLS {{RFC8446}}, SSH {{RFC4253}}, and IKE {{RFC7296}}. It can also be used as public-key encryption such as HPKE {{RFC9180}}, where a KEM is used to establish shared secret for symmetric key encryption. The security considerations in this document are intended to assist protocol designers and implementers in deploying NTRU+ securely in such environments.
