...
The encapsulation algorithm (see Section 6.3.1 of {{KpqC2025}}) takes as input an n-bit random message, which SHOULD be generated by an approved RBG, where n denotes the degree of the underlying polynomial in the ring R. The algorithm outputs a ciphertext c and a shared secret K. 
...
The ciphertext is computed as c = hr + m, where h is the public key, r is a short polynomial derived from the intermediate randomness r, and m is encoded as a polynomial using a semi-generalized one-time pad (SOTP) operation. Both r and m are short polynomials with coefficients sampled according to the CBD. The resulting ciphertext c is represented in NTT form. This representation is preserved during transmission and is used directly as input to the decapsulation algorithm.   

After encapsulation is completed, the n-bit random message SHOULD be securely erased. 

### Decapsulation

The decapsulation algorithm (see Section 6.3.1 of {{KpqC2025}}) takes as input a ciphertext c and the private key sk = (f, h^{-1}, H(PK)). The algorithm outputs a shared secret K or an error symbol. Using f and h^{-1} in the private key, both r and m are recovered from the ciphertext c  
