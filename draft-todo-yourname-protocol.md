...
Historically, achieving worst-case correctness error has been a significant challenge in NTRU-based public-key encryptions, where an adversary can choose r and m with its own for a ciphertext c = hr+m. To achieve such a worst-case correctness error, NTRU+ uses two techniques: r is generated by the FO transform and m is generated by the SOTP encoding, and thus the adversary cannot have control over choosing r and m adversararilly. This result builds upon {{DHK21}} and extends it to the CBD sampling used in NTRU+. As ohter lattice-based KEMs, for any honestly generated ciphertext, decapsulation failures can leak information about sk, but NTRU+ has a neglible worst-case correctness error so that such a decapsulation failure never hapapens in reality over one sk and all decapsulation keys.  
