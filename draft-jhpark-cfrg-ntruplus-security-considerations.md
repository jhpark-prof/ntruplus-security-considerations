...
As with other lattice-based KEMs, observable decapsulation failures may potentially leak information about the private key. Therefore, correctness failures are relevant not only to reliability but also to security.

In ML-KEM and similar LWE/Module-LWE-based schemes, the decryption-error condition is governed by noise and rounding terms, and the message does not directly affect the failure expression. This makes it possible to analyze correctness essentially independently of the adversary's choice of message.

In NTRU-based public-key encryption schemes, the situation is more subtle. For ciphertexts of the form c = hr + m, decryption involves both the randomness r and the message representative m. The use of a Fujisaki-Okamoto (FO) transform can make it difficult for an adversary to directly control r in valid ciphertexts, but the message component may still affect the decryption-error condition.

To address this issue, NTRU+ uses SOTP encoding. The random message m is encoded into a polynomial M through the SOTP encoding procedure, and M is used in the ciphertext computation. This encoding is designed so that the coefficients of M follow the same CBD distribution as the randomness polynomial r. As a result, an adversary no longer has direct control over the values of r and M appearing in honestly generated ciphertexts.

This correctness analysis builds upon {{DHK22}} and extends the analysis to the CBD sampling used in NTRU+. Consequently, the worst-case correctness error of NTRU+ does not substantially differ from the average-case correctness error over honestly generated ciphertexts. NTRU+ is designed so that this failure probability is negligible for honestly generated ciphertexts.
