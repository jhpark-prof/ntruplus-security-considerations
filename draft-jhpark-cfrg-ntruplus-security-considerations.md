...
Following Lyubashevsky and Seiler {{LS19}}, NTRU+ operates over a polynomial ring R parameterized by (n,q), where n denotes the degree of a cyclotomic trinomial and q denotes the coefficient modulus. The use of a cyclotomic trinomial enables efficient polynomial multiplication through the Number Theoretic Transform (NTT). Secret polynomials are sampled according to a centered binomial distribution (CBD), where each coefficient is generated by subtracting one uniformly random bit from another, resulting in values in {-1,0,1}.

### Key Generation

The key generation algorithm (see Section 6.3.1 of {{NTRUPlus26}}) takes as input two independent 32-byte random seeds, which SHOULD be generated by an approved random bit generator (RBG), and produces a public key pk and a private key sk. The two seeds are used to generate two short polynomials f' and g, respectively, whose coefficients are sampled according to the CBD.
...
The NTRU+ key-generation algorithm does not include a Pair-wise Consistency Test (PCT). Although a PCT could be incorporated as an optional self-test by extending the key-generation procedure, it is not part of the current specification. This document therefore considers only the standard key-generation procedure defined in {{NTRUPlus26}}.

### Encapsulation

The encapsulation algorithm (see Section 6.3.1 of {{NTRUPlus26}}) requires an n-bit random message as input. The message SHOULD be generated using an approved RBG. The algorithm outputs a ciphertext c and a 32-byte shared secret K.
...
The decapsulation algorithm (see Section 6.3.1 of {{NTRUPlus26}}) takes as input a ciphertext c and the private key sk = (f, h^{-1}, H(PK)). The algorithm outputs either a 32-byte shared secret K for a valid ciphertext or a decapsulation error for an invalid ciphertext. Thus, NTRU+ employs explicit rejection for invalid ciphertexts rather than deriving and returning a pseudorandom key.
...
Implementations SHOULD ensure that the computation of H(r) is performed in constant time and does not reveal information about its input through timing, power consumption, cache access, or other side channels.
