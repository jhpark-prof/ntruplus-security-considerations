...
The key generation algorithm (see Section 6.3.1 of {{KP26}}) takes as input two independent 32-byte random seeds, which SHOULD be generated by an approved random bit generator (RBG), and produces a public key pk and a private key sk. The two seeds are used to generate two short polynomials f' and g, respectively, whose coefficients are sampled according to the CBD.
